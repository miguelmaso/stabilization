...
\end{equation}
\Miguel{
where $n$ is the Manning coefficient. It defines the resistance to flow by the roughness of the bottom or other factors and it is determined empirically. In practice, the Manning coefficient varies from 0.01 for a very smooth bed (concrete) to 0.05 for a rough bed (rocks) \cite{chow1988}
}
...
\pder{\bm{\phi}}{t} + \mathbf{A}_i\pder{\bm{\phi}}{x_i}
 - \pder{}{x_k}\left(\mathbf{K}_{ik}\pder{\bm{\phi}}{x_i}\right) + \mathbf{S}\bm{\phi} + \Miguel{ \mathbf{b}_i\pder{z}{x_i} } = 0
\end{equation}
...
\begin{equation*}
    ...
    \end{matrix}\right]
\end{equation*}
CONVÉ AFEGIR AQUESTA EQUACIÓ ENUMERADA?
}
...
\mathbf{r} \defeq 
  \pder{\bm{\phi}}{t} + \mathbf{A}_i\pder{\bm{\phi}}{x_i}
  -\pder{}{x_k}\left(\mathbf{K}_{ik}\pder{\bm{\phi}}{x_i}\right) + \mathbf{S}\bm{\phi} + \mathbf{b}_i\pder{z}{x_i} \qquad i,k\in\{1,n_d\}
\end{equation}
...
\int_\Omega \left(
    ...
    + \pder{\omega_k}{x_j} \mathbf{K}_{jk} \pder{\bm{\phi}}{x_i} + \omega_k \mathbf{S}\bm{\phi} + \omega_k \mathbf{b}_i\pder{z}{x_i}
\right) d\Omega\\ +
\int_\Omega \frac{\beta l^e}{\lambda} \left(
    ...
    \left.
    + \pder{\omega_k}{x_j} \mathbf{A}_j(\mathbf{S}\bm{\phi} + \mathbf{b}_i\pder{z}{x_i})
\right) d\Omega
=0
\end{multline}
...
\displaystyle\mathbf{T}_F^{ab} &= \int_{\Omega_e} \frac{\beta l^e}{2} \pder{N_a}{x_i}\mathbf{A}_i \Miguel{\mathbf{b}_i \pder{z_b}{x_i}} d\Omega
\nonumber
\end{align}
