...
The solution of this implicit system requires an iterative procedure. We have used the Newton-Raphson procedure, is defined in an incremental way as
...
In this section we present four different examples. Three of them are oriented to verify a single aspect of the procedure explained in this research,and the last one is devoted to test all the capabilities of the formulation in a practical case with experimental data.
...
The problem is discretized with a mesh fine enough to test the artificial diffusion added by the stabilization (see figure \ref{step_mesh}). The average element size is $0.06m$ and near the corner the mesh is refined to $0.02m$.
The time step is set automatically to keep a Courant number equal to $1.0$ at every step. The problem is executed three times with different algorithmic constants $\beta = 0.001$, $0.01$ and $0.1$. In that example, the shock capturing term is disabled.
...
The time step is set automatically to keep a Courant number equal to $1.0$ at every step. The problem is executed three times with different algorithmic constants $\beta = 0.001, 0.01, 0.1$. In this case, the shock capturing term is switched off.
...
The domain is discretized with a mesh with an average element size of $0.05m$. There are 115.000 elements and the time step is computed to keep a courant number of 1.0. In figure \ref{experiment_mesh} there are two details of the mesh, near the dam and around the building.
