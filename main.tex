...
\usepackage{anyfontsize}   % to avoid font size substitution warnings
\usepackage{array}         % extending tabular environment
\usepackage{subcaption}    % sub-figures
\usepackage{float}         % placing figures
\usepackage{xcolor}

\usepackage[normalem]{ulem} % package for underlying. Remove after the revision.
...
%In two dimensions, the eigenvalues of the matrix $\alpha_1\mathbf{A}_1 + \alpha_2\mathbf{A}_2$ are $\lambda_{1,3} = \alpha_1u_1 + \alpha_2u_2 \pm c$ and $\lambda_2 = \alpha_1u_1 + \alpha_2u_2$.
...
}%The first issue is the computation of the velocity field,
\Miguel{
Since the computation of the velocity is the result of dividing
}
the discharge by the water height, this operation is ill-conditioned in the dry regions. In this research, the velocity field is computed in a two step procedure. First of all, the \Miguel{\sout{velocity} inverse of the water depth} is computed at each element following the next expression, initially proposed by Kurganov \cite{kurganov2007}:
...
\Miguel{$z'$ which is obtained imposing the equilibrium}%
\begin{equation}
...
This function makes possible to construct the source system vector $\mathbf{T}$ with a linear combination between $z$ and $z'$.

\vspace{10pt}
\noindent\Miguel{\fbox{\parbox{.97\textwidth}{
Nota: a la pràctica, això és irrellevant, pel fet de que no és un esquema monotònic. És a dir, si hi ha una discontinuitat, hi hauran oscilacions i aquest petit truc no ho pot solucionar. En canvi, el \emph{shock capturing} és molt efectiu en aquesta regió. Hauria sigut molt millor eliminar aquest paràgraf abans d'enviar-ho a la revista, però ja que un revisor pregunta sobre això, no vull eliminar-lo.}
}}
...
The stabilized method proposed is not monotonic and the dry domain model is acting to ensure stability, but it is not providing monotonicity. It is important to note the all the modifications have been done at the momentum balance, so that the mass conservation is done by the weak principle, but mass sign preservation is not guaranteed.
}

\Miguel{
Both primary unknowns, water depth and flow rate, are continuous at the dry-wet interface, but its derivatives are discontinuous. Here the shock capturing will mitigate the spurious oscillations, but the order of accuracy will be lost due to the non-linear artificial diffusion \cite{badia2014}. 
}
...
\Miguel{
We study the propagation of a wave in a channel with a backward step (Figure \ref{step_mesh}) and 1m depth. An initial perturbation in the free surface at the left wall generates a wave which travel from 0 to 6s. The initial perturbation reads
\begin{equation}
...
\Miguel{
Te wave is reflected at the right wall and faces the step in the opposite direction. 
All the boundary conditions are set slip.
}
