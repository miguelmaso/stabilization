...
% \Miguel{\sout{, with $i=1,3$. while the number of dimensions $n_d$ is equal to 2}}
\Miguel{. Since this formulation is defined in a two dimansional space ($n_d=2$), in the following we will consider $i=1,2$}.
...
% \Miguel{\sout{
% Equation (\ref{general_sw}) satisfies the standard boundary conditions in space
% }}
% \Miguel{
% \begin{align*}
...
%\Miguel{\sout{and then integrating by parts,}}
...
\begin{figure}[H]
    ...
    \caption{Parabolic basin. Mass conservation error. The total mass is integrated over all the domain (black) and over the wet domain (blue). The mesh used in this simulation is the intermediate one, with element size of $0.05m$.}
    \label{parabola_mass}
\end{figure}

Figure \ref{parabola_results} shows the results using the finest mesh. The discretization is not shown for the seek of simplicity. As expected, there is no variation on the results in the transversal section.
