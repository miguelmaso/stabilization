...
\Ignasi{\sout{In modeling t}T}he shallow water equations for hydrodynamics \Ignasi{have} traditionally \Ignasi{\sout{have}} been \Ignasi{modelled \sout{used} using} finite volumes [cite Riemann] because of its advantages of stability and monotonicity. \Ignasi{\sout{Because of} Given} its geometric flexibility and its natural way to introduce high order schemes, finite elements have been applied too [some citations]. In order to prevent \Ignasi{numerical} instabilities in finite elements\Ignasi{,} stabilization \Ignasi{\sout{[citations]}} of finite order of interpolation \Ignasi{\sout{[Taylor-Hood]}} can be explored \Ignasi{[Taylor-Hood, OTHER CITATIONS]}. More recently, the discontinuous Galerkin \Ignasi{technique} has been introduced \Ignasi{[CITATIONS] \sout{,}. It \sout{which}} has the advantages of the geometrical flexibility of the finite elements and the stability of the finite volumes, but the introduction of high order schemes is not straightforward. In this research we are interested in classical stabilized finite elements with equal interpolation order for all the variables and we will explore the capabilities of the finite increment calculus (FIC) to develop stable formulations for the shallow water equations.

Several families of stabilization can be found in the literature, the most relevant are SUPG\Ignasi{[CITATIONS]}, ASGS\Ignasi{[CITATIONS]}, GLS\Ignasi{[CITATIONS]} and FIC\Ignasi{[CITATIONS]}. Due to the hyperbolic character of the shallow water equations, a particular stabilization method for compressible flow or the Euler equations need to be developed. The FIC-based stabilization has been applied to convection-diffusion, incompressible flows and turbulent flows \cite{onate1998,onate2001}\Ignasi{T'HE MODIFICAT AQUESTA CITA PER A QUE APAREGUIN VARIES CITES AGRUPADES (AMB EL PAQUET CITE ES POT FER. E.G. [2-4]) \sout{, i}. I}n \Ignasi{\sout{that}those} cases\Ignasi{,} where the convective term has an important role, a first order FIC term is enough to provide stability to the system. However, the shallow water equations are governed by the convective term and the wave equation in \Ignasi{a} mixed formulation \cite{codina2008} \Ignasi{and}, in consequence\Ignasi{,} the common derivation of the FIC-based stabilization is not enough to provide stability in all the range of applicability of the shallow water equations. A generalization of this method is proposed in order to provide a global stability for the shallow water equations. Other possibilities of the FIC-based formulations are explored to provide a shock capturing stabilization. \Ignasi{AQUESTA ULTIMA FRASE NO L'HE ENTES. VOLS DIR QUE HAS EXPLORAT TERMES D'ALTRES FORMULACIONS FIC PER A ESTABILITZAR EL SHOCK CAPTURING? PODRIES POSAR AIXO EN UN ALTRE PARAGRAF I PARLAR DE L'ESTAT DE L'ART EN AQUESTA QUESTIO EN CONCRET}

Additionally\Ignasi{,} the dry domain requires an accurate modeling because the hyperbolic equations \Ignasi{require} positive water depth in all the domain \Ignasi{HE AFEGIT LA PARAULA require, REVISA SI AIXI S'ENTEN EL QUE VOLIES DIR}. Among several proposals \cite{heniche2000,candy2017,barros2011} a modified depth integration \cite{defina2000} is performed in combination with the FIC stabilization. \Ignasi{FALTA COMPLETAR UNA MICA L'ESTAT DE L'ART D'AQUEST PUNT}

\Ignasi{AL FINAL HAURIES DE POSAR 1 PARAGRAF EXPLICANT COM S'ORGANITZA EL PAPER: PRIMER PRESENTES LES EQUACIONS DE GOVERN, DESPRES L'ESTABILITZACIO, EXEMPLES, ETC.}

\section{Governing equations and linearization}

\Ignasi{Let us \sout{We}} start \Ignasi{\sout{considering} presenting} the shallow water equations\Ignasi{\sout{, which come from the}. They are the result of} depth \Ignasi{\sout{integrated} integrating the} Navier-Stokes equations \Ignasi{\sout{and},} assuming the vertical velocity and its acceleration \Ignasi{\sout{be}} negligible \cite{zien3}. \Ignasi{HAURIES D'EXPLICAR SI ESCRIUS LES EQUACIONS AMB VARIABLES CONSERVATIVES I PER QUÈ HO FAS AIXI AMB CITES} \Ignasi{Their general expression can be written as} 
...
\Phi = \Phi_0
\end{equation}
where $\Phi_0$ are the initial water height and velocities, verifying Equation (\ref{general_sw})\Ignasi{,} and the \Ignasi{suitable} boundary conditions
...
The bottom friction \Ignasi{DEFINEIX (AQUÍ O ABANS) BOTTOM FRICTION COM $\tau^b_{31}$} is modelled with the Manning formula \Ignasi{\sout{extended to} generalized for} two dimensions

\begin{equation}
\frac{\tau^b_{31}}{\rho} = -gn^2\frac{\abs{\mathbf{q}}\mathbf{q}}{h^{\sfrac{7}{3}}}
\end{equation}

The averaged horizontal stresses are \Ignasi{calculated from the} combination of the molecular stresses and the Reynolds stresses as follows
...
\end{equation}
where the matrices $\mathbf{A}_i$ and $\mathbf{K}_{ik}$ are the linearization matrices of the convective fluxes and the diffusive fluxes. The convective matrix \Ignasi{ET REFEREIXES A Ai?} is obtained \Ignasi{after} applying the chain rule to the vector of fluxes $\mathbf{F}_i$
...
\end{matrix}\right]
\end{equation}
In the following sections, the rain, atmospheric pressure gradient and the wind friction will be neglected \Ignasi{SI AQUESTS TERMES JA HAN APAREGUT ABANS, ELS HAURIES D'HAVER DEFINIT, ENCARA QUE DESPRES NO APAREGUIN MÉS}.
