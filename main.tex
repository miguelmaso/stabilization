...
%More recently, the discontinuous Galerkin \Ignasi{technique} has been introduced \Ignasi{[CITATIONS] \sout{,}. It \sout{which}} has the advantages of the geometrical flexibility of the finite elements and the stability of finite volumes, but the introduction of high order schemes is not straightforward.
...
An Eulerian approach based on the work of \cite{leclerc1990} and \cite{heniche2000} is presented. \Ignasi{FALTA COMPLETAR UNA MICA L'ESTAT DE L'ART D'AQUEST PUNT} \Miguel{ESTAT DE L'ART AMPLIAT. ENCARA ES PODRIA PARLAR DELS MÈTODES LAGRANGIANS}
...
In the one dimensional case ($n_d=1$) and scalar balance ($n_b=1$)\Ignasi{$n_d=1$ POTSER HAS DE DEFINIR AQUEST $n_d$ AL PRINCIPI AMB L'EQUACIO 1}\Miguel{FET}, the FIC-based stabilization is based on a modified version of the governing equations as \cite{onate1998}
...
The time step is set automatically to keep a Courant number equal to $1.0$ at every step. The problem is solved three times with different algorithmic constants: $\beta = 0.001, 0.01, 0.1$. In this case, the shock capturing term is switched off.
...
\Ignasi{LA FIGURA \ref{waves_propagation} NO ESTA CITADA A L'ARTICLE. TOTES LES FIGURES HAN DE SER CITADES AL MENYS 1 COP.ELS PEUS DE FIGURA PODEN SER MES BREUS AIXI...}
...
\Ignasi{LA FIGURA \ref{chanel_geometry} NO ESTA CITADA A L'ARTICLE.}
...
\Ignasi{LA TAULA \ref{gauges_positions} NO ESTA CITADA A L'ARTICLE.}
...
%\Ignasi{NO ENTENC PERQUE PARLES DE L'ESTABILITZACIO VMS AQUI. HA DE QUEDAR CLAR QUINA ES L'APORTACIO DEL PAPER EN RELACIO A L'ESTABILITZACIO PRESENTADA} \Miguel{AQUÍ ESTIC PARLANT DEL LIMITS DE L'APORTACIÓ.}
